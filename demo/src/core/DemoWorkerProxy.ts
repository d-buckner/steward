import { EventBus, EventHandler, EventSubscription } from '@d-buckner/steward'
import { Message, MessageDefinition, generateMessageId } from '@d-buckner/steward'
import { WorkerOptions } from '@d-buckner/steward'
...
/**
 * Demo-specific WorkerProxy that uses local worker file to avoid MIME type issues
 */
export class DemoWorkerProxy<
...
> implements EventBus {
  ...
  constructor(
    ...
    // Copy decorator metadata from service class to this proxy
    // This is needed for createServiceActions to work
    if (serviceClass.__messageTypes) {
      ;(this as any).__messageTypes = serviceClass.__messageTypes
    }
    if (serviceClass.__actionCreators) {
      ;(this as any).__actionCreators = serviceClass.__actionCreators
    }
  ...
  private async initializeWorker(): Promise<void> {
    try {
      // Create worker with the demo-specific worker file
      this.worker = new Worker(
        new URL('../workers/steward-worker.ts?worker', import.meta.url),
  ...
  // Add the action methods directly to support createServiceActions
  startProcessing(items: number[], operation: 'sum' | 'fibonacci' | 'prime_count') {
    return this.send('START_PROCESSING', { items, operation })
  }

  cancelProcessing() {
    return this.send('CANCEL_PROCESSING', {})
  }
