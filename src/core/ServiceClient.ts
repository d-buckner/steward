...
import type { TypedServiceToken } from './ServiceTokens';
...
export class ServiceClient<T extends Service> {
  private eventBus = new ServiceEventBus<T['state']>();
  private subscriptions = new Map<keyof T['state'], Set<(value: any) => void>>();
      ...
      get: (target, prop, receiver) => {
        if (typeof prop === 'string') {
          ...
          if (target.isServiceMethod(prop)) {
            return (...args: unknown[]) => target.callMethod(prop, args);
          }

          // Handle other service properties
          if (prop in target.getServiceInstance()) {
            const service = target.getServiceInstance();
            const value = (service as Record<string, unknown>)[prop];
  ...
  on<K extends keyof T['state']>(key: K, callback: (value: T['state'][K]) => void): { unsubscribe: () => void } {
    ...
    this.trackSubscription(key, callback);

    return {
      unsubscribe: () => {
        this.eventBus.off(key, callback);
        this.untrackSubscription(key, callback);
      }
    };
  }
  ...
  once<K extends keyof T['state']>(key: K, callback: (value: T['state'][K]) => void): void {
    const wrappedCallback = (value: T['state'][K]) => {
      callback(value);
      this.untrackSubscription(key, wrappedCallback);
    };
    this.eventBus.once(key, wrappedCallback);
    this.trackSubscription(key, wrappedCallback);
  }
  ...
  /**
   * Call a method on the service (fire-and-forget)
   */
  private callMethod(methodName: string, args: unknown[]): void {
    if (this.workerId) {
      this.callWorkerMethod(methodName, args);
    } else {
      this.callLocalMethod(methodName, args);
    }
  }
  ...
  private setupLocalEventForwarding(): void {
    ...
    // Get all state keys and subscribe to each one individually
    const stateKeys = Object.keys(service.getState());

    stateKeys.forEach(key => {
      service.on(key, (value: T['state'][keyof T['state']]) => {
        this.emit(key, value);
      });
    });
  ...
  private setupWorkerEventForwarding(): void {
    ...
    this.getWorker()?.addEventListener('message', this.workerMessageHandler);
  }
   ...
   */
  private callLocalMethod(methodName: string, args: unknown[]): void {
    const service = this.getServiceInstance();
  ...
  /**
   * Call method on worker service (fire-and-forget)
   */
  private callWorkerMethod(methodName: string, args: unknown[]): void {
    const worker = this.getWorker();

    if (!worker) {
      console.error(`Worker not found for service ${this.token.id}`);
      return;
    }

    // Send message to worker (fire-and-forget)
    worker.postMessage({
      type: 'METHOD_CALL',
      methodName,
      args
   ...
   */
  private trackSubscription<K extends keyof T['state']>(key: K, callback: (value: any) => void): void {
   ...
   */
  private untrackSubscription<K extends keyof T['state']>(key: K, callback?: (value: any) => void): void {
